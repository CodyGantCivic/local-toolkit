// ==UserScript==
// @name         CivicPlus - Multiple Item Upload
// @namespace    http://civicplus.com/
// @version      1.0.0
// @description  Allows posting a single CivicAlert item to multiple categories in CivicPlus admin pages.
// @author       CivicPlus
// @match        *://*/admin/civicalerts.aspx*
// @match        *://*/Admin/civicalerts.aspx*
// @grant        none
// @run-at       document-end
// ==/UserScript==
...
(function () {
  ...
  window.MultipleItemUpload = {
    ...
    init: async function () {
      ...
      try {
        // Only run on the CivicAlerts admin page (case-insensitive)
        const path = (window.location.pathname || '').toLowerCase();
        if (!path.endsWith('/admin/civicalerts.aspx')) {
          return;
        }

        /**
         * Wait for a DOM element matching the selector. Resolves with the element
         * or null after timeout milliseconds. Uses MutationObserver for efficient detection.
  ...
  window.MultipleItemUpload.init();
})();